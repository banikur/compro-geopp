...
const sectors = [
  {
    title: 'Energi & PLTA',
    color: 'bg-sky/10 border-sky/30 text-sky',
    icon: '\u26A1',
    clients: ['PLN Nusantara Power \u2013 PLTA Bakaru', 'PLN NP \u2013 PLTA Brantas', 'Poso Energy \u2013 Poso'],
  },
  {
    title: 'Pertambangan',
    color: 'bg-flare/10 border-flare/30 text-flare',
    icon: '\u26CF\uFE0F',
    clients: [
      ...
      <section className="py-24 bg-cloud">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0}
            className="mb-16"
          >
            <p className="text-xs font-medium uppercase tracking-widest text-sky mb-3">Berdasarkan Sektor</p>
                ...
                <ul className="space-y-3">
                  {sector.clients.map((c) => (
                    <li key={c} className="flex items-start gap-2 text-sm text-ink">
                      <Building2 className="w-4 h-4 text-steel shrink-0 mt-0.5" />
                      {c}
                    </li>
                  ))}
                </ul>
      ...
      <section className="py-24 bg-navy text-white">
        <div className="max-w-7xl mx-auto px-6">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} custom={0}
            className="mb-16"
          >
            <p className="text-xs font-medium uppercase tracking-widest text-sky mb-3">Apa Kata Klien</p>
            ...
            {testimonials.map((t) => (
              ...
              >
                <div className="flex gap-1">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 text-flare fill-flare" />
                  ))}
                </div>
                <p className="text-steel leading-relaxed text-sm flex-1">&ldquo;{t.quote}&rdquo;</p>
                <div className="flex items-center justify-between pt-4 border-t border-steel/20">
                  <div>
                    <p className="text-white font-semibold text-sm">{t.author}</p>
                    <p className="text-steel text-xs">{t.company}</p>
                  </div>
                  <span className="px-3 py-1 bg-sky/20 border border-sky/40 text-sky text-xs rounded-full font-medium">
                    {t.result}
                  </span>
                </div>
              </motion.div>
